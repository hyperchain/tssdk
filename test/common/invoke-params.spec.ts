...
describe("HvmAbiBuilder", () => {
  test("basic test using InvokeBean in setHash.abi", () => {
    const abiBuffer: Buffer = fs.readFileSync(
      path.resolve(__dirname, "../resource/hvm-jar/setHash/setHash.abi")
    );

    const paramsBuilder = new InvokeParams.HvmAbiBuilder(
      abiBuffer,
      BeanType.InvokeBean,
  ...
  test("basic test using MethodBean in setHash.abi", () => {
    const abiBuffer: Buffer = fs.readFileSync(
      path.resolve(__dirname, "../resource/hvm-jar/setHash/setHash.abi")
    );

    const paramsBuilder = new InvokeParams.HvmAbiBuilder(abiBuffer, BeanType.MethodBean, "getHash");
  ...
  test("basic test (parameters of the same type) (int)", () => {
  ...
  test("basic test (parameters of the same type) (String)", () => {
    const abiBuffer = fs.readFileSync(
      path.resolve(__dirname, "../resource/hvm-jar/Basic-crossContractB/hvm.abi")
    );
    const params = new InvokeParams.HvmAbiBuilder(
      abiBuffer,
      HvmType.BeanType.MethodBean,
    ...
    expect(params.build().getParams()).toBe(
      "fefffbce00087365744d794d61700010000000046a6176612e6c616e672e537472696e676b6579790010000000066a6176612e6c616e672e537472696e6776616c756565"
    );
  });
  ...
  test("boundary-complement overload", async () => {
    ...
    {
      const params = new InvokeParams.HvmAbiBuilder(abiBuffer, BeanType.MethodBean, "getX()");
    ...
    {
      const params = new InvokeParams.HvmAbiBuilder(abiBuffer, BeanType.MethodBean, "getX(int)");
      params.addParam(123);
      const transaction = new Transaction.HVMBuilder(account.getAddress(), providerManager)
        .invoke(contractAddress, params.build())
        .build();
  ...
  test("deploy and invoke sum", async () => {
    const jarBuffer: Buffer = fs.readFileSync(
      path.resolve(
        __dirname,
        "../resource/hvm-jar/boundary-complement/target/boundary-complement-1.0-table.jar"
  ...
  test("deploy and invoke setMyMap", async () => {
    const jarBuffer: Buffer = fs.readFileSync(
      path.resolve(
        __dirname,
        "../resource/hvm-jar/Basic-crossContractB/Basic-0.0.1-SNAPSHOT-crossContractB.jar"
      )
    );
    const abiBuffer: Buffer = fs.readFileSync(
      path.resolve(__dirname, "../resource/hvm-jar/Basic-crossContractB/hvm.abi")
...

    {
      ...
      logger.info(params.build().getParams());
      // 4. 签名
      transaction.sign(account);
      // 5. 通过 contract service 部署
      const invokeRequest = contractService.invoke(transaction);
...
describe("HvmTypeBuilder", () => {
  test("setHash contract", async () => {
    ...
    // getSize()
    {
      const params = new InvokeParams.HvmTypeBuilder("getSize");
      const transaction = new Transaction.HVMBuilder(account.getAddress(), providerManager)
        .invoke(contractAddress, params.build())
        .build();
      transaction.sign(account);
    ...
    // getHash(test)
    {
      ...
      const params = new InvokeParams.HvmAbiBuilder(abiBuffer, BeanType.MethodBean, "getHash");
  ...
  test("contractcollection-2.0-SNAPSHOT contract", async () => {
    ...
    {
      const params = new InvokeParams.HvmTypeBuilder("add");
    ...
    {
      const params = new InvokeParams.HvmTypeBuilder("get");
