// copy from https://github.com/fidm/asn1/blob/master/test/pem.ts
...
export enum PemType {
  ...
  AnyPrivateKey = "PRIVATE KEY",
}

export interface PemJSON {
  type: PemType;
  body: Buffer;
...
export class PEM {
   ...
   */
  body: Buffer;
  private headers: Record<string, string>; // Optional headers.
  constructor(t: string, body: Buffer) {
    this.type = t as PemType;
    this.body = body;
   ...
   */
  get procType(): string {
    return this.getHeader(PROC_TYPE);
  }
   ...
   */
  toString(): string {
    let rVal = PEM_START + this.type + PEM_END_OF_LINE + "\n";
    ...
    rVal += PEM_END + this.type + PEM_END_OF_LINE + "\n";
    return rVal;
  }
...
function parse(lines: string[]): PEM {
  ...
  if (line == null || !line.startsWith(PEM_START) || !line.endsWith(PEM_END_OF_LINE)) {
    throw new Error("pem: invalid BEGIN line");
  }
  const type = line.slice(PEM_START.length, line.length - PEM_END_OF_LINE.length);
  if (type === "") {
  ...
  let body = "";
  while (line != null && !line.startsWith(PEM_END)) {
    body += line;
    line = lines.shift();
  }
  if (line == null || line !== `${PEM_END}${type}${PEM_END_OF_LINE}`) {
