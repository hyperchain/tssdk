...
import { CurveParams, KeyPairHex } from "./types";
 ...
 * 获取椭圆曲线参数
 */
function getGlobalEcParams(): CurveParams {
  return { curve, G, n };
}
 ...
 * 生成ecparam
 */
function generateEcparam(): CurveParams {
  // 椭圆曲线
  ...
  if (G == null) throw new Error("decode point hex not supported");
 ...
 */
function generateKeyPairHex(a?: number, b?: number, c?: RandomGenerator): KeyPairHex {
 ...
 */
function getKeyPairfromPrivateKey(privateKey: string): KeyPairHex {
  const d = new BigInteger(privateKey, 16);
  const P = G.multiply(d); // P = dG，p 为公钥，d 为私钥
  const Px = leftPad(P.getX().toBigInteger().toString(16), 64);
  const Py = leftPad(P.getY().toBigInteger().toString(16), 64);
  const publicKey = "04" + Px + Py;
 ...
 * 生成压缩公钥
 */
function compressPublicKeyHex(s: string) {
...
function utf8ToHex(input: string) {
  ...
  // 转换到字数组
  const words: Uint8Array = Uint8Array.from([]);
 ...
 * 转成utf8串
 */
function arrayToUtf8(arr: string[]) {
  const words: Uint8Array = Uint8Array.from([]);
  let j = 0;
 ...
 * 转成字节数组
 */
function hexToArray(hexStr: string) {
  const words = [];
 ...
 * 验证公钥是否为椭圆曲线上的点
 */
function verifyPublicKey(publicKey: string) {
 ...
 * 将公钥解析为椭圆曲线点
 */
function decodeCurvePointFromPk(pk: string): ECPointFp {
  const point = curve.decodePointHex(pk);
  if (!point) throw new Error("decode curve point from public key failed");
  const x = point.getX();
  const y = point.getY();
  if (!y.square().equals(x.multiply(x.square()).add(x.multiply(curve.a)).add(curve.b))) {
    throw new Error("verify curve failed");
  }
  return point;
}
 ...
 * 验证公钥是否等价，等价返回true
 */
function comparePublicKeyHex(publicKey1: string, publicKey2: string) {
 ...
 * 将x,y数据解析为椭圆曲线点
 */
function decodeCurvePointFromUint8Array(ox: Uint8Array, oy: Uint8Array): ECPointFp {
  const point = curve.decodeNativePointFromUint8Array(ox, oy);
  if (!point) throw new Error("decode curve point from uint8 array failed");
  const x = point.getX();
  const y = point.getY();
  if (!y.square().equals(x.multiply(x.square()).add(x.multiply(curve.a)).add(curve.b))) {
    throw new Error("verify curve failed");
