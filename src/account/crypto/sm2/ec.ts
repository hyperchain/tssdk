...
export class ECPointFp {
  private curve: ECCurveFp;
  private x: ECFieldElementFp | null;
  private y: ECFieldElementFp | null;
  private z: BigInteger;
  private zinv: BigInteger | null;
  constructor(
    curve: ECCurveFp,
  ...
  add(b: ECPointFp) {
    ...
    const zInv = z3.modInverse(q);
    return new ECPointFp(
      this.curve,
      this.curve.fromBigInteger(x3.multiply(zInv).mod(q)),
      this.curve.fromBigInteger(y3.multiply(zInv).mod(q))
    );
  }
  ...
  toString() {
    const Px = leftPad(this.x!.toBigInteger().toString(16), 64);
    const Py = leftPad(this.y!.toBigInteger().toString(16), 64);
    return "04" + Px + Py;
  }
  to65() {
...
export class ECCurveFp {
  public q: BigInteger;
  public a: ECFieldElementFp;
  public b: ECFieldElementFp;
  public infinity: ECPointFp;
  ...
  decodePointHex(s: string) {
    switch (parseInt(s.substr(0, 2), 16)) {
      ...
      case 3:
        // 压缩
        const x = this.fromBigInteger(new BigInteger(s.substr(2), 16).mod(this.q));
  ...
  /**
   * 解析仅包含x,y数据的 Uint8Array为椭圆曲线点
   */
  decodeNativePointFromUint8Array(x: Uint8Array, y: Uint8Array) {
    return new ECPointFp(
      this,
      this.fromBigInteger(new BigInteger(ByteUtil.toHex(x), 16)),
      this.fromBigInteger(new BigInteger(ByteUtil.toHex(y), 16))
    );
  }
}
